...
BioCalc <- function(folder, type='ascii') {
	
	if (tolower(type) %in% c("ascii", "grd", "bil", "")) {
		if (tolower(type) == 'ascii') {
			ext <- ".asc"
			fmt <- "ascii"
		} else if (tolower(type) == 'grd') {
			ext <- ".grd"
			fmt <- "raster"
		} else if (tolower(type) == 'bil') {
			ext <- ".bil"
			fmt <- "bil"
		} else {
			ext <- ""
	...
	TxList <- LoadMonthlyFiles(folder, varbl='tmax', type=fmt)
	TnList <- LoadMonthlyFiles(folder, varbl='tmin', type=fmt)
	TaList <- LoadMonthlyFiles(folder, varbl='tmean', type=fmt)
	PrList <- LoadMonthlyFiles(folder, varbl='prec', type=fmt)
